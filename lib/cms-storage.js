...
const SOCIAL_KV_KEY = "portfolio:social:v1";
...
async function readSocial(socialFile) {
  const redis = getRedis();
  if (redis) {
    const raw = await redis.get(SOCIAL_KV_KEY);
    const data = normalizeRedisJson(raw);
    if (data && typeof data.posts === "object") return data;
  }
  return readJsonSafe(socialFile, { posts: {} });
}

async function writeSocial(socialFile, data) {
  const redis = getRedis();
  if (redis) {
    await redis.set(SOCIAL_KV_KEY, data);
    return;
  }
  await atomicWriteJson(socialFile, data);
}
